...
func TestPipeline_New(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:     "postgres",
			database: _postgres,
			want:     &engine{client: _postgres},
		},
		{
			name:     "sqlite",
			database: _sqlite,
			want:     &engine{client: _sqlite},
		},
	}

	// run tests
	for _, test := range tests {
		got := New(test.database, 0)
...
func testPostgres(t *testing.T) (*engine, sqlmock.Sqlmock) {
	...
	return &engine{client: _postgres, compressionLevel: 3}, _mock
}
...
func testSqlite(t *testing.T) *engine {
	...
	return &engine{client: _sqlite, compressionLevel: 3}
}
